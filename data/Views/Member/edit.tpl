...
<form action="{url action="update" id=$data.id}" method="POST" class="container-fluid row">
		...
		<div class="col-12 col-md-6 col-lg-5">
			...
			<div>
				<select name="client" id="e{counter skip=1}" class="form-select">
					...
					<option value="{$code}"{if $code eq $data.client} selected{/if}>{$client.name}</option>
					{/foreach}
				</select>
				<div class="invalid-feedback"></div>
			</div>
			...
			<div class="col-12 col-md-4" data-categories="l">
				<select name="categories[]" class="form-select" id="e{counter skip=1}">
					...
					{foreach from=$categoriesL key="code" item="category"}
						<option value="{$code}"{if $code eq $data.categories[0]} selected{/if}>{$category.name}</option>
					{/foreach}
				</select>
				<div class="invalid-feedback"></div>
			</div>
					...
					<fieldset disabled hidden>
						...
						<select name="categories[]" class="form-select" data-parent="{$parent}">
							...
							{foreach from=$categories key="code" item="category"}
								<option value="{$code}"{if $code eq $data.categories[1]} selected{/if}>{$category.name}</option>
							{/foreach}
						</select>
						{/foreach}
					</fieldset>
				...
				<div data-categories="s">
					...
					<fieldset disabled hidden>
						...
						<select name="categories[]" class="form-select" data-parent="{$parent}">
							...
							<option value=""></option>
							{foreach from=$categories key="code" item="category"}
								<option value="{$code}"{if $code eq $data.categories[2]} selected{/if}>{$category.name}</option>
							{/foreach}
						</select>
						{/foreach}
					</fieldset>
	...
	<div class="col-12">
		<div data-name="targets">
			...
			<div class="form-check form-check-inline">
				<input type="checkbox" name="targets[]" id="e{counter skip=0}" value="{$code}" class="form-check-input"{if $code|in_array:$data.targets} checked{/if} /><label for="e{counter skip=1}" class="form-check-label">{$target.name}</label>
			</div>
			{/foreach}
		</div>
		<div class="invalid-feedback"></div>
	...
	<div class="col-12">
		<div data-name="medias">
			...
			<div class="form-check form-check-inline">
				<input type="checkbox" name="medias[]" id="e{counter skip=0}" value="{$code}" class="form-check-input"{if $code|in_array:$data.medias} checked{/if} /><label for="e{counter skip=1}" class="form-check-label">{$media.name}</label>
			</div>
			{/foreach}
		</div>
		<div class="invalid-feedback"></div>
