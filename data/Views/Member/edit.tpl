...
<nav class="navbar navbar-light bg-light">
	<h2 class="container-fluid px-4">編集登録画面</h2>
</nav>
{/block}
...
{block name="body"}
<form action="{url action="update" id=$data.id}" method="POST" class="form-grid-12">
	...
	<div class="grid-colspan-12 grid-colreset">
		<label class="form-label d-flex gap-2">媒体<span class="badge bg-danger">必須</span></label>
	...
	<div class="grid-colspan-12 grid-colreset">
		...
		<textarea name="content" id="e{counter skip=1}" class="form-control" rows="3" placeholder="入力してください">{$data.content|escape:"html"}</textarea>
		<div class="invalid-feedback"></div>
	</div>
	<div class="grid-colspan-12 grid-colreset">
		...
		<div class="form-grid-12 grid-row-gap-2" data-name="keyword">
			{for $foo=0 to 5}
			<div class="grid-colspan-6"><input type="text" name="keyword[]" id="e{counter skip=1}" class="form-control" placeholder="入力してください"{if $foo|array_key_exists:$data.keywords} value="{$data.keywords[$foo]|escape:"html"}"{/if} autocomplete="off" /></div>
			{/for}
		</div>
		<div class="invalid-feedback"></div>
	</div>
	...
	<div class="grid-colspan-12 grid-colreset">
		...
		<div>
			<div data-name="files">
				<div hidden>{foreach from=$data.files item="file"}
	...
	<div class="grid-colspan-12 grid-colreset">
		...
		<div class="d-flex flex-wrap gap-3" id="thumbnail" data-bs-toggle="modal" data-bs-target="#thumbnailModal">
			<div class="card mb-3 d-flex text-center card-select">ファイルを選択</div>
			<img class="thumbnail" alt="サムネイル" src="/file/thumbnail/{$data.id}.png" />
		</div>
		<div class="invalid-feedback"></div>
	</div>
	<div class="grid-colspan-12 grid-colreset">
